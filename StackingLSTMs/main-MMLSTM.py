...
from NaiveLSTM import NaiveLSTM as Model
...
def do_train(model, train_data):
    ...
    for item in train_data:
        solution = torch.tensor([item['solution']]).to(args.device)
...
def test(model, test_data):
    ...
    with torch.no_grad():
        for item in test_data:
            ...
            pred_for_test = model.linear(h_t_1)
...
for i in range(1,11):
    if __name__ == '__main__':
        ...
        model = NaiveLSTM().to(args.device)
